...
/*regras que servem para indicar o grau de relação entre os filmes da empresa*/

distribuidora(X, Y) :- distribuidora_genero(X, K),tipo(Y, K). /* indica que X é distribuidora de Y se ele for distribuidora de um genero K e Y for do tipo K */
distriubuidora(X, Y) :- vice_presidente(X),distribuidora_tipo(Y,_). /* indica que o vice presidente será distribuidora de todos os distribuidoras de genero */
superior(X, Y) :- distribuidoras(X, Y). /* essa regra é um caso base da relação de cargo superior,onde X é superior a Y se X for distribuidor de Y */
superior(X, Y) :- distrubuidora_genero(X,_), tipo(Y,_).
subordinado(X, Y) :- distribuidoras(Y, X). /*essa regra é um caso base de subordinação,onde X é subordinado a Y se Y for distribuidor de X*/
subordinado(X, Y) :- distribudoras(K, X), superior(Y, K). /*indica que X é subordinado a Y se alguém K que seja distribuidor de X, também seja subordinado a Y*/
